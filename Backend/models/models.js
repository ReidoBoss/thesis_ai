...
Users.getUserImage = (user_id, result) => {
    sql.query(
        "SELECT  user_id, image FROM users WHERE user_id= ? ", [user_id],
        (error, queryResult) => {
            if (error) {
...
Users.getSingleOperator = (operator_id, result) => {
    sql.query(
        "SELECT   operator_id, user_id, first_name,last_name, gender, birthdate, address, email,mobile_number FROM operator WHERE operator_id= ? ", [operator_id],
        (error, queryResult) => {
            if (error) {
...
Users.getSingleResponder = (operator_id, result) => {
    sql.query(
        "SELECT   responder_id, user_id, department,institution, mobile_number, email, city,zipcode, address FROM responder WHERE responder_id= ? ", [operator_id],
        (error, queryResult) => {
            if (error) {
                console.log("Error in executing operator table query", error);
                result(error, null);
...
Users.getSinglePost = (post_id, result) => {
    sql.query(
        "SELECT  description, timestamp, emergency_type, city, zipcode, address FROM post WHERE post_id= ? ", [post_id],
        (error, queryResult) => {
            if (error) {
                console.log("Error in executing property_table query", error);
                result(error, null);
...
Post.getSinglePostReport = (report_id, result) => {
    sql.query(
        "SELECT  post_id, responder_id, operator_id, additional_description FROM post_report WHERE report_id= ? ", [report_id],
        (error, queryResult) => {
            if (error) {
                console.log("Error in executing property_table query", error);
                result(error, null);
...
Post.editPostStatus = (postId, newStatus, result) => {
    sql.query(
        "UPDATE post SET status = ? WHERE post_id = ?", [newStatus, postId],
        (error, results) => {
            if (error) {
                console.log("error: ", error);
                result(error, null);
...
Post.updatePost = (updatePost, result) => {
    sql.query(
        "UPDATE post_report SET responder_id = ?, operator_id = ?, additional_description = ? WHERE post_id = ?", [updatePost.responder_id, updatePost.operator_id, updatePost.additional_description, updatePost.post_id],
        (error, results) => {
            if (error) {
                console.log("error: ", error);
                result(error, null);
